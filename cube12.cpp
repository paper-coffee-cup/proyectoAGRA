...
bool verBit2(int n, int pos, int bits) {
  return n & (1 << (bits - pos));
}

int swap2(int n, int pos, int bits) {
  return  n ^ (1 << (bits - pos));
}

int assign(int n, bool flag, int pos, int bits) {
  int ans;
  if (flag)
    ans = n | (1 << (bits - pos));
  else
...
int fn(int r, int c, int C) {
  return r * C + c;
}

pair<int, int> f(int k, int C) {
  int r = k / C;
  int c = k - r * C;
  return make_pair(r, c);
}

pair<bool, pair<int, int>> move(int d, int rc, int R, int C) {
